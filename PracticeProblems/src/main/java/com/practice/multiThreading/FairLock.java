...
/**
 *
 * The lock() method is no longer declared synchronized.
 * Instead only the blocks necessary to synchronize are nested inside synchronized blocks.
 *
 ...
 * The queueObject.doWait() call is placed outside the synchronized(this) block to avoid nested monitor lockout
 * , so another thread can actually call unlock() when no thread is executing inside the synchronized(this) block in lock() method.
 *
 ...
 *
 *
 * Created by abhi.pandey on 11/23/14.
 */
